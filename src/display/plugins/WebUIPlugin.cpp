...
void WebUIPlugin::setup(Controller *_controller, PluginManager *_pluginManager) {
    ...
    if (controller->isSDCard()) {
        // Fork: stage the controller image on the SD card (the T8 headless build has no LittleFS).
...
void WebUIPlugin::setupServer() {
    ...
    // Fork: the storage FS is chosen once in Controller (LittleFS, SD_MMC, or SD over SPI on the T8).
...
void WebUIPlugin::handleSettings(AsyncWebServerRequest *request) const {
    if (request->method() == HTTP_POST) {
        controller->getSettings().batchUpdate([request](Settings *settings) {
            ...
            if (request->hasArg("displayRotated"))
                settings->setDisplayRotated(parseBoolArg(request->arg("displayRotated")));
            ...
            // Fork: Discord shot feedback (direct mode) and the Gaggibot bridge.
            if (request->hasArg("discord"))
                settings->setDiscord(parseBoolArg(request->arg("discord")));
            ...
            if (request->hasArg("discordAi"))
                settings->setDiscordAi(parseBoolArg(request->arg("discordAi")));
    ...
    doc["displayRotated"] = settings.isDisplayRotated();
